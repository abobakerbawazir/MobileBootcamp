...
use App\Models\Typetransactiont;
use App\Trails\ApiResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class TypetransactiontController extends Controller
{
    use ApiResponse;
    /**
    ...
    public function store(Request $request)
    {
        ...
        $result = Typetransactiont::create($request->all());
    ...
    {
        $result = Typetransactiont::find($id);
        if (!is_null($result)) {
            return $this->success_ressponse(result: $result);
        } else {
    ...
    public function rules(Request $request)
    {
        return Validator::make($request->all(), [
            'name' => 'required|unique:typetransactionts',
